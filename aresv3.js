...
function onTextBefore(userobj, text) {  
    ...
    if (userobj.customName && userobj.customName !== "") {  
        ...
        htmlContent = convertImageURLsToImages(htmlContent);  // NEW: Convert image URLs
        ...
        htmlText = convertImageURLsToImages(htmlText);  // NEW: Convert image URLs
...
function convertImageURLsToImages(text) {  
    // Regex to match URLs ending in common image extensions
    // Handles URLs with query parameters (like ?cid=something)
    var imageUrlRegex = /(https?:\/\/[^\s<>"]+?\.(?:gif|jpg|jpeg|png|webp|bmp|svg))(\?[^\s<>"]*)?/gi;  
    
    var result = text.replace(imageUrlRegex, function(match) {  
        // Clean the URL (remove any trailing punctuation that's not part of the URL)
        var cleanUrl = match.replace(/[.,;!?]+$/, '');
        
        return '<img src="' + cleanUrl + '" style="max-width:300px; max-height:300px; vertical-align:bottom;" border="0" />';  
    });  
...
var ems = [  
    ...
    for (var j = 0; j < sortedEms.length; j++) {  
        ...
        if (typeof imageIndex !== 'undefined') {  
            var replacement = '<img src="https://cdn.jsdelivr.net/gh/Shipo-bit/assest@main/imagenes/org/' +  
                              imageIndex + '.gif" style="max-width:19px; max-height:19px; vertical-align:bottom;" border="0" />';  
    ...
    for (var k = 0; k < gifShortcuts.length; k++) {  
        ...
        var replacementGif = '<img src="https://cdn.jsdelivr.net/gh/Shipo-bit/assest@main/imagenes/gifs/' +  
                             word + '.gif" style="max-width:19px; max-height:19px; vertical-align:bottom;" border="0" />';  
